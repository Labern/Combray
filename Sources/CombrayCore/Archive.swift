...
public struct Archive: Sendable {
    ...
    public func applyTranscription(_ r: TranscriptionResult, toLetterId letterId: String) throws -> Letter {
        ...
        letter.title = Self.clean(r.title)
            ?? Self.composedTitle(sender: r.sender, recipients: r.recipients, summary: r.summary)
            ?? letter.title
    ...
    /// Builds a "Letter to … from … about …" title from extracted fields when the model gave none.
    static func composedTitle(sender: String, recipients: [String], summary: String) -> String? {
        let s = clean(sender)
        let r = recipients.compactMap(clean)
        guard s != nil || !r.isEmpty else { return nil }
        var parts = ["Letter"]
        if !r.isEmpty { parts.append("to \(r.joined(separator: ", "))") }
        if let s { parts.append("from \(s)") }
        if let summary = clean(summary) {
            parts.append("about \(summary.split(separator: " ").prefix(8).joined(separator: " "))")
        }
