...
public struct TranscriptionResult: Decodable, Sendable, Hashable {
    ...
    enum CodingKeys: String, CodingKey {
        case transcription, title, summary, sender, recipients, date, people_mentioned, uncertain_spans, meta
    }
}
...
public struct AnthropicClient: Sendable {
    ...
    public func transcribe(imageURLs: [URL]) async throws -> TranscriptionResult {
    ...
    Then extract: a title written in the form "Letter to [recipient] from [sender] about [short \
    subject]" (fill in what you can from the contents); a brief 1–3 sentence summary of what the letter is \
    about; the sender; the recipient(s); the date (use a date written on the letter if present with \
    source "written", otherwise infer from the contents with source "inferred", else source \
    "unknown") with a confidence; and any people mentioned. Use the partial date form you can \
    support: "1962", "1962-03", or "1962-03-04".
    ...
    static var schema: [String: Any] { [
        ...
        "required": ["transcription", "title", "sender", "recipients", "date", "people_mentioned", "uncertain_spans"],
        "properties": [
